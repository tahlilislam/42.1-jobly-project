...
describe("PATCH /users/:username", () => {
  ...
  test("bad request if invalid data (same user)", async function () {
    const resp = await request(app)
      .patch(`/users/u1`)
      .send({
        firstName: 42,
  ...
  test("works: set new password (same user)", async function () {
    const resp = await request(app)
      .patch(`/users/u1`)
      .send({
  ...
  test("works: set new password (admin user)", async function () {
    const resp = await request(app)
      .patch(`/users/u1`)
      .send({
    ...
    const isSuccessful = await User.authenticate("u1", "new-password-admin");
    expect(isSuccessful).toBeTruthy();
  });
});
